...
export const SERVER_INSTRUCTIONS = `You are connected to the Khan Academy MCP server, which provides access to Khan Academy's extensive educational content library. Use the following tools to help users learn, explore topics, and study effectively.
...
### Practice & Assessment
- **get_exercise** — Get details about a specific Khan Academy exercise, including its practice URL, related videos/articles to study first, and course placement. Use when a student wants to practice a skill.
...
### Study & Review
- **study_guide** — Generate a structured study guide for a given topic. Aggregates key concepts, practice exercises, and video references into one convenient overview. Ideal when a user is preparing for a test or wants a summary of a unit.
...
export function registerAllTools(server: McpServer, client: KhanClient): void {
